...
dem_sr <- readr::read_csv(here::here(data_path, "CODA_dem_data.csv")) %>% 
  dplyr::select(-`...1`) %>% 
  tidyr::pivot_longer(!HRS_ID, names_to = "time", values_to = "dem") %>% 
  dplyr::mutate(Year = dplyr::case_when(time == "dem98" ~ 1998,
                                        time == "dem00" ~ 2000,
                                        time == "dem02" ~ 2002,
                                        time == "dem04" ~ 2004,
                                        time == "dem06" ~ 2006,
...
checking <- cognitiondata %>% 
  dplyr::filter(Year < 2021) %>% 
  dplyr::left_join(dem_sr, by = c("HRS_ID", "Year")) %>% 
  dplyr::select(HRS_ID, Year, dem)

id_convert <- cognitiondata %>% 
  dplyr::filter(Year < 2021) %>% 
  dplyr::left_join(dem_sr, by = c("HRS_ID", "Year")) %>% 
  dplyr::select(HRS_ID, Year, dem) %>% 
  dplyr::group_by(HRS_ID) %>% 
  dplyr::summarize(dem = mean(dem)) %>% 
  dplyr::filter(dem > 0)
