...
fs::dir_create(Mplus_path, "170_time_metric_quad")
setwd(here::here(Mplus_path, "170_time_metric_quad"))
...
longcog_modelwt <- mplusObject(
  MODEL = "
           ...
           ! boost effect
           b BY y0@0 y2@1 y4@1 y6@1 y8@1 y10@1 y12@1 y14@1 y16@1;
           b BY y18@1 y20@1 y22@1;
           b@0;
           [b];
           ...
           ! covariates
           i s q b on femc nhwc educ baseagec;
  ...
  VARIABLE = "IDVARIABLE = mplusid;
              ...
              CATEGORICAL = y0-y22;",
  OUTPUT = "TECH4; SVALUES; STDYX;",
  usevariables = colnames(mplusdata),
  rdata = mplusdata,
)

dr_longwt <- mplusModeler(longcog_modelwt, modelout = "quad_boost_time_metric.inp", run = TRUE)
...
longcog_modelwt <- mplusObject(
  ...
  male75s = ms + 2*mq*.5;
  fem75s = (ms + 2*mq*.5) + fem75;
  
  fem75c = (ms + s1) + 2*(mq + q1)*.5;
